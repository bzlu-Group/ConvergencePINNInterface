...
from Tool import grad, data_transform, gradient,data_transform_theta_interface,data_transform_theta_boundary
...
def f_grad(x,label,device):
    ...
    if label=='inner':  
        ...
        f=(-(xt[0]+xt[1])*torch.cos(xt[0]+xt[1])+2*xt[0]*xt[1]*torch.sin(xt[0]+xt[1])).view(-1,1)-u(x,label)

    elif label=='out':
        f=(2*(xt[0]+xt[1])*torch.sin(xt[0]+xt[1])+2*(xt[0]**2+xt[1]**2)*torch.cos(xt[0]+xt[1])).view(-1,1)-u(x,label)       
    else:
        raise ValueError("invalid label for u(x)")
...
def test_data_net(device):  
    
    step=0.15
    x = np.arange(-3, 3+step, step)
    y = np.arange(-3, 3+step, step)
    xx,yy=np.meshgrid(x,y)
    input_x=torch.tensor(xx).view(-1,1)
    input_y=torch.tensor(yy).view(-1,1)
    input=(torch.cat((input_x,input_y),1))
    rr=torch.tensor(torch.sqrt((input_x**2+input_y**2)))
    
    theta=-1000*torch.ones_like(input_x)
    # I
    index_x=torch.where(input_x>=0)[0]
    index_y=torch.where(input_y>=0)[0]
    loca=list(set(index_x.numpy())&set(index_y.numpy()))
    theta[loca]=torch.arccos(input_x[loca]/rr[loca])
    # II
    index_x=torch.where(input_x<0)[0]
    index_y=torch.where(input_y>0)[0]
    loca=list(set(index_x.numpy())&set(index_y.numpy()))
    theta[loca]=pi-torch.arccos(-input_x[loca]/rr[loca]) 
    # III
    index_x=torch.where(input_x<0)[0]
    index_y=torch.where(input_y<0)[0]
    loca=list(set(index_x.numpy())&set(index_y.numpy()))
    ...
    index_y=torch.where(input_y<0)[0]
    loca=list(set(index_x.numpy())&set(index_y.numpy()))
    theta[loca]=2*pi-torch.arccos(input_x[loca]/rr[loca])      
    
    r_in=r_interface(theta)
    r_out=r_boundary(theta)
    index_inner=torch.where(rr<r_in)[0]
    inner=input[index_inner,:]  

    index_out1=torch.where(rr<=r_out)[0]
    out=input[index_out1,:]
    rr=rr[index_out1,:]
    r_in=r_in[index_out1]
    index_out=torch.where(rr>r_in)[0] 
    out=out[index_out,:]
    ...
    if -1000 in theta:
        raise ValueError('-1000 in theta')
...
def main(args):
    ...
    test_out,test_inner=test_data_net(device)   
    ...
    ### train data
    data=Data_Single(device=device)
    ...
    x_in_b,y_in_b,input_in_b=data_transform_theta_interface(inner_b_theta,device) 
 
    out_b_theta=data.sampleFromBoundary(args.train_out_b) 
    out_b_theta= out_b_theta.clone().detach().requires_grad_(True)  
    x_outb,y_outb,out_b=data_transform_theta_boundary(out_b_theta,device)
    ...
    for epoch in range(args.nepochs):      
        ...
        ff1=-(U_1xx+U_1yy)-U1-f_grad(input_in,'inner',device)
        loss_in=torch.mean((ff1)**2)

        ff1_dx=grad(-(U_1xx+U_1yy)-U1,x_in,device)
        ff1_dy=grad(-(U_1xx+U_1yy)-U1,y_in,device)
        ...
        ff2= -(U_2xx+U_2yy)-U2-f_grad(input_out,'out',device)  
        loss_out=torch.mean((ff2)**2)

        ff2_dx=grad(-(U_2xx+U_2yy)-U2,x_out,device)
        ff2_dy=grad(-(U_2xx+U_2yy)-U2,y_out,device)

        lipsch_omega2=torch.max(torch.cat((ff2_dx**2,ff2_dy**2),dim=0))
        del ff2_dx,ff2_dy,ff2,U_2xx,U_2yy,U_2x,U_2y,U2
        ##5=========================================
        ob=net_out(out_b)
